...
function runHealthCheck(){
    ...
    var duration = 600;
    var queryString = "names[]=Custom/onejl.api.ProductApi#getProduct(String).500/Count&names[]=Custom/onejl.api.ProductApi#getProduct(String).502/Count"+
    "&names[]=Custom/onejl.api.ProductApi#getProduct(String).503/Count&names[]=Custom/onejl.api.ProductApi#getProduct(String).504/Count"+
    ...
    request({
        ...
        method : 'GET',
        qs : {
            'names[]': 'Custom/onejl.api.ProductApi#getProduct(String).500/Count',
            'period' : 600,
            'from' : tenMinutesAgo,
            'to' : currentTime
        }
    ...
    metric_data.metrics.forEach(function(metric){
        ...
        metric.timeslices.forEach(function(slice){
            ...
            if(errPerMinute > ERROR_THRESHOLD){
                console.log("   # " + metricName + " & error rate " + errPerMinute);
                log_message = log_message + "   # " + metricName + " & error rate " + errPerMinute + ".<br/>";
                console.log("GetProducts API error exceeding threshold with error rate " + errPerMinute);
                log_message = log_message + "GetProducts API error exceeding threshold with error rate " + errPerMinute + ".<br/>";
...
function triggerAlert(log_message){
  ...
  //})
  var subject = '1JL GetProducts API Error Limit breached: Raise P3 Inc [Callout:JLWEBSUPP]';
