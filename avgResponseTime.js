...
var ERROR_THRESHOLD = 5; //in seconds
var NRQL = "SELECT average(duration) FROM Transaction where appName='OneJL Browse (Live)' since 5 minutes ago";
...
function checkAverageTime(){
  ...
  url: 'https://insights-api.newrelic.com/v1/accounts/1075077/query',
    ...
    if (!error && response.statusCode == 200) {
      ...
      //console.log(jsonObj);
      var duration = jsonObj.results[0].average;
      var log_message = "The alert condition of avg Response Time Limit breached was triggered -<br/><br/>";
      console.log("page load average duration now is " + duration);
      if(!isAlertsPeriodSilent()){
        if(duration > ERROR_THRESHOLD){
          console.log("Page load time has increased, higher than threshold. it is " + duration);
          log_message = log_message + "Page load time has increased, higher than threshold. it is " + duration + ".<br/>";
...
function triggerAlert(log_message){
  ...
  //})
  var subject = '1JL Average Page Response Time (All Servers) > 5 sec for 5 min: Raise P3 Inc [Callout:JLWEBSUPP]';
