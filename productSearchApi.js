...
function runHealthCheck(){
    ...
    request({
        ...
        method : 'GET',
        qs : {
            'names[]': 'Custom/onejl.api.ProductSearchApi#keywordSearch(String,String,String,int,int).500/Count',
            'period' : 600,
            'from' : tenMinutesAgo,
            'to' : currentTime
        }
...
function checkIfErrorCountIsOk(response_object){
    ...
    metric_data.metrics.forEach(function(metric){
        metric.timeslices.forEach(function(slice){
            ...
            var log_message = "The alert condition of product search API Errors Limit breached was triggered -<br/><br/>";
            if(errPerMinute > ERROR_THRESHOLD){
                console.log("Product Search API error exceeding threshold with error rate " + errPerMinute);
...
function _execute(){
    console.log("starting the execution of ProductSearchApi.js...");
...
function triggerAlert(log_message){
  ...
  //})
  var subject = '1JL Product Search API (keyword search) Error threshold Breached: Raise P3 Inc [Callout:JLWEBSUPP]';
