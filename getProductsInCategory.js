...
function runHealthCheck(){
    ...
    request({
        ...
        method : 'GET',
        qs : {
      'names[]': 'Custom/onejl.api.ProductApi#getProductsInCategory(String,String,String,int,int).500/Count',
      'period' : 600,
      'from' : tenMinutesAgo,
      'to' : currentTime
    }
...
function checkIfErrorCountIsOk(response_object){
    ...
    metric_data.metrics.forEach(function(metric){
        metric.timeslices.forEach(function(slice){
            ...
            var log_message = "The alert condition for Get Products category API Errors Limit breached was triggered -<br/><br/>";
            console.log(errPerMinute);
            if(errPerMinute > ERROR_THRESHOLD){
                console.log("GetProducts in Category API error exceeding threshold with error rate " + errPerMinute);
...
function _execute(){
    console.log("starting the execution of getProductsInCategory.js...");
...
function triggerAlert(log_message){
  ...
  //})
  var subject = '1JL GetProducts In Category API Error Limit Breached: Raise P3 Inc [Callout:JLWEBSUPP]';
